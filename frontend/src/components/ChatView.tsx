...
export const ChatView: React.FC = () => {
  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col items-center justify-center p-2 sm:p-4 md:p-6 lg:p-8 min-h-0">
      {/* Centered Conversation Container (~750–880px) with intentional editorial framing */}
      <div className="w-full max-w-[840px] xl:max-w-[880px] h-full flex flex-col overflow-hidden rounded-2xl border border-[#E8E4DB] bg-[#FFFFFF] shadow-[0_4px_24px_rgba(28,25,23,0.04)]">
        {/* Chat Room Subheader */}
        <div className="flex items-center justify-between border-b border-[#E8E4DB] bg-[#FBFAF7] px-4 sm:px-6 py-3 shrink-0">
          <div className="flex items-center space-x-2.5 sm:space-x-3">
            <span className="font-serif text-base sm:text-lg font-medium text-[#1C1917]">
              {partner?.name}
            </span>
            <span className="text-[11px] font-mono text-[#8C857B]">/ private stream</span>
          </div>
          <span className="text-[11px] font-mono text-[#059669] flex items-center gap-1.5">
        ...
        {/* Message Stream */}
        <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 bg-[#FBFAF7]">
          {isLoading ? (
            ...
            messages.map((msg) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 flex items-center space-x-1 px-1 text-[11px] font-mono text-[#A8A29E]">
                    <span>
                    ...
                    {msg.is_me && (
                      <span>
                        {msg.read_at ? (
                          <CheckCheck className="h-3.5 w-3.5 text-[#C2410C]" />
                        ) : (
                          <Check className="h-3.5 w-3.5 text-[#A8A29E]" />
                        )}
                      </span>
                    )}
                  </div>
        ...
        {/* Replying Banner */}
        {replyingTo && (
          <div className="flex items-center justify-between border-t border-[#E8E4DB] bg-[#F5F2EB] px-4 sm:px-6 py-2.5 text-xs sm:text-sm shrink-0">
            <div className="flex items-center space-x-2 truncate">
              <Reply className="h-4 w-4 text-[#C2410C] shrink-0" />
              <span className="font-semibold text-[#1C1917] shrink-0">
                Replying to {replyingTo.is_me ? 'yourself' : replyingTo.sender?.name || partner?.name}:
              </span>
              <span className="truncate italic text-[#78716C]">"{replyingTo.content}"</span>
            </div>
            <button
              onClick={() => setReplyingTo(null)}
              className="p-1 text-[#8C857B] hover:text-[#1C1917] transition-all"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        {/* Input Composer */}
        <form onSubmit={handleSendMessage} className="border-t border-[#E8E4DB] bg-[#FFFFFF] p-3.5 sm:p-4 shrink-0">
            ...
            <textarea
              ...
              placeholder={
                replyingTo
                  ...
                  : `Write to ${partner?.name || 'partner'}... (Enter to send)`
              }
              rows={1}
              className="flex-1 max-h-32 min-h-[44px] resize-none rounded-xl border border-[#D4CEC2] bg-[#FBFAF7] px-4 py-3 text-xs sm:text-sm text-[#1C1917] placeholder-[#A8A29E] focus:border-[#C2410C] focus:bg-[#FFFFFF] focus:outline-none focus:ring-1 focus:ring-[#C2410C]"
            />
            <button
              type="submit"
              disabled={!inputText.trim() || isSending}
              className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#1C1917] text-white hover:bg-[#C2410C] disabled:opacity-30 transition-all shrink-0 shadow-sm"
