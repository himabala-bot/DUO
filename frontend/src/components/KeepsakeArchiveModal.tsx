...
export const KeepsakeArchiveModal: React.FC<KeepsakeArchiveModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 sm:p-6 backdrop-blur-[3px] overflow-y-auto">
      <div className="relative w-full max-w-4xl rounded-3xl border border-[#EFE8DC] bg-[#FFFFFF] shadow-[0_16px_48px_rgba(66,47,14,0.12)] overflow-hidden flex flex-col max-h-[90vh]">
        {/* Top Header */}
        <div className="flex items-center justify-between border-b border-[#EFE8DC] bg-[#FAF7F2] px-6 py-4 shrink-0">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h2 className="font-serif text-xl sm:text-2xl font-bold text-[#422F0E]">Keepsake Archive</h2>
              <p className="text-[11px] font-mono text-[#A89F91]">Past daily love prompts answered together</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto p-5 sm:p-7 bg-[#FFFFFF]">
          {isLoading ? (
            <div className="flex h-64 items-center justify-center text-xs sm:text-sm font-mono text-[#A89F91]">
              Opening scrapbook...
            </div>
          ) : historyList.length === 0 ? (
            <div className="rounded-3xl border border-[#EFE8DC] bg-[#FAF7F2] p-8 text-center space-y-2">
              <Heart className="h-8 w-8 text-[#FCC4C0] mx-auto mb-1" />
              <h4 className="font-serif text-xl text-[#422F0E]">No past reflections yet</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                    ...
                    return (
                      <button
                        ...
                        className={`min-w-[160px] md:min-w-0 md:w-full text-left rounded-2xl p-3.5 transition-all flex items-center justify-between border shrink-0 ${
                          isSelected
                            ? 'border-[#422F0E] bg-[#FFFFFF] shadow-sm ring-2 ring-[#FCC4C0]/40 font-semibold text-[#422F0E]'
                      ...
                      >
                        <div className="truncate">
                          ...
                          <p className="mt-0.5 text-[11px] text-[#A89F91] truncate">{item.summary}</p>
                        </div>
                        <ChevronRight className={`h-4 w-4 shrink-0 ${isSelected ? 'text-[#EA5E86]' : 'text-[#A89F91]'}`} />
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="md:col-span-8">
                {isLoadingDetails ? (
                  <div className="flex h-64 items-center justify-center rounded-3xl border border-[#EFE8DC] bg-[#FAF7F2] text-xs font-mono text-[#A89F91]">
                    Opening entry...
                  </div>
                ) : dateResponses ? (
                  <div className="space-y-4">
                    <div className="pb-3 border-b border-[#EFE8DC] flex items-center space-x-2">
                      <Calendar className="h-4 w-4 text-[#EA5E86]" />
                      <h3 className="font-serif text-lg sm:text-xl font-bold text-[#422F0E]">
                    ...
                    <div className="space-y-4">
                      {dateResponses.questions.map((q, idx) => {
                        ...
                        return (
                          <div
                            key={q.id}
                            className="rounded-3xl border border-[#EFE8DC] bg-[#FAF7F2] p-4 sm:p-5 space-y-3"
                          >
                            <h4 className="font-serif text-sm sm:text-base font-bold text-[#422F0E]">
                              {idx + 1}. {q.question}
                            </h4>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                              {/* My Answer */}
                              <div className="rounded-2xl border border-[#EFE8DC] bg-[#FFFFFF] p-3.5 text-xs text-[#422F0E]">
                              ...
                              {/* Partner's Answer */}
                              <div className="rounded-2xl border border-[#FCC4C0]/40 bg-[#FFF8FA] p-3.5 text-xs text-[#422F0E]">
                                <span className="text-[10px] font-mono uppercase tracking-widest text-[#EA5E86] block mb-1">
